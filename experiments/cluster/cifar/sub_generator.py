...
if __name__ == "__main__":
    ...
    if args.forward or args.back:
        ...
        file_path = f'event_subs/{fname}_{args.rate}.sub'
        with open(file_path, 'w') as f:
            f.write(f"executable = /usr/bin/python3\n")
            f.write(f"arguments = main.py --{fname} --rate {float(args.rate)/100}\n")
            f.write(f'error = errs/{fname}_{args.rate}.err\n')
            f.write(f'output = outs/{fname}_{args.rate}.out\n')
            f.write(f'log = logs/{fname}_{args.rate}.log\n')
            f.write(f'request_memory = 64096\nrequest_cpus = 1\nrequest_gpus = 1\nqueue')
    else:
        if args.avg: fname = 'avg'
        elif args.prox: fname = 'prox'
        elif args.admm: fname = 'admm'
        file_path = 'subs/'+fname+f'_{args.rate}.sub'
        with open(file_path, 'w') as f:
            f.write(f"executable = /usr/bin/python3\n")
            f.write(f"arguments = main.py --{fname} --rate {float(args.rate)/100}\n")
            f.write(f'error = errs/{fname}_{args.rate}.err\n')
